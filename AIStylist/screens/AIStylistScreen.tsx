...
export default function AIStylistScreen() {
  ...
  useEffect(() => {
    if (isHandsFreeMode && isConversationActive) {
      console.log('🎤 Starting VAD for hands-free mode...');
      ...
      vadDetector.startMonitoring((event) => {
        ...
        } else if (event.type === 'speech_end' && event.duration) {
          console.log(`🎤 VAD: Speech ended (${event.duration}ms), stopping recording...`);
          if (isRecording) {
            stopHoldToSpeak();
          }
        }
  ...
  }, [isHandsFreeMode, isConversationActive, isRecording, isProcessing]);

  const toggleHandsFreeMode = useCallback(async () => {
    ...
    if (newMode) {
      ...
      await vadDetector.stopMonitoring();
    }
  }, [isHandsFreeMode]);
  ...
  const startSpeechRecognition = useCallback(async () => {
    // 🚫 Disable Web Speech API on mobile - it's not reliable
    // Use hold-to-speak recording instead
    if (Platform.OS !== 'web') {
  ...
  const handleVoicePress = useCallback(() => {
    // 🎤 Mobile: Only use hold-to-speak (press and hold)
    // 🎤 Web: Can use click-to-speak OR hold-to-speak
    
    if (Platform.OS !== 'web') {
      // On mobile, do nothing on tap - user must press and hold
      console.log('🎙️ Mobile: Use hold-to-speak (press and hold the button)');
      return;
    }

    // Web platform can use click-to-speak
    if (isListening) {
  ...
  const getAIResponseWithImageAndVoice = useCallback(async (voiceText: string) => {
    ...
    try {
      console.log('⚡ ALEXA-MODE: Starting AI response with streaming...');

      // 🚀 PHASE 1: INSTANT ACKNOWLEDGMENT (0ms perceived latency!)
      const instantAck = streamingHandler.getInstantAcknowledgment(voiceText);
      console.log('⚡ Instant acknowledgment:', instantAck);

      // Play instant acknowledgment immediately
      if (Platform.OS !== 'web') {
        speakTextLocal(instantAck).catch(err => console.warn('Failed to speak ack:', err));
      }

      // Add instant acknowledgment to UI
      const ackMessage = createChatMessage('assistant', instantAck);
      setMessages(prev => [...prev, ackMessage]);

      // 🧠 CHECK CONTEXT: Resolve references like "this", "that", "the blue one"
      const contextInfo = contextManager.resolveReference(voiceText);
      if (contextInfo.hasReference && contextInfo.contextHint) {
        console.log('🧠 Context resolved:', contextInfo.contextHint);
      }
      ...
      // Add context manager's summary
      const contextSummary = contextManager.buildContextPrompt();
      const enhancedHistory = contextSummary ? `${contextSummary}\n\nCurrent:\n${conversationHistory}` : conversationHistory;

      // Update conversation context for continuous chat
      setConversationContext(enhancedHistory);
...
Keep responses conversational and natural, as if you're talking to them in person. Be encouraging and constructive.`;
      ...
      if (response) {
        ...
        // 🧠 STORE IN CONTEXT MANAGER for future reference resolution
        contextManager.addExchange(voiceText, response, {
          imageUrl: imageReference || undefined,
        });
        ...
        <View style={[styles.overlay, { paddingTop: insets.top }]}>
          ...
          {isConversationActive && (
            <View style={styles.quitButtonContainer}>
              ...
              >
                <Mic size={18} color={isHandsFreeMode ? Colors.success : Colors.white} />
                <Text style={[
                  styles.handsFreeButtonText,
                  isHandsFreeMode && styles.handsFreeButtonTextActive
                ]}>
                  {isHandsFreeMode ? '🎤 Hands-Free' : 'Hands-Free'}
                </Text>
              </TouchableOpacity>
